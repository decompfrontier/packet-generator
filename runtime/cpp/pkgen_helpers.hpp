...
#if __STDCPP_FLOAT32_T__ > 0 || __STDCPP_FLOAT64_T__ > 0 // for compatibility with Clang for Android NDK
#include <stdfloat>
#endif
...
using chrono_time = std::chrono::time_point<std::chrono::system_clock, std::chrono::milliseconds>;

static constexpr bool string_to_chrono(const std::string& in, chrono_time& out) {
...
static constexpr bool chrono_to_string(const chrono_time& in, std::string& out) {
	try {
#if __cplusplus <= 201703L
...
static constexpr bool chrono_to_unix(const chrono_time& in, uint64_t& out) {
	try {
		out = std::chrono::duration_cast<std::chrono::seconds>(in.time_since_epoch()).count();
		return true;
	}
	catch (...) {
		return false;
...
template <typename T>
using inner_type_v = T::value_type;
template <typename T>
